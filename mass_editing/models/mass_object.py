...
class MassObject(orm.Model):
    ...
    _columns = {
        ...
        'field_ids': fields.one2many('mass.object.field', 'mass_id',
                                     'Fields'),
...
class field_override(orm.Model):
    _name = "mass.object.field"
    _order = "sequence"

    _columns = {
        'mass_id': fields.many2one('mass.object', 'Mass Edit', required=True),
        ...
        ## Relateds from field_id
        "name": fields.related(
            "field_id", "name", string="Name", type="char",
            readonly=True),
        "ttype": fields.related(
            "field_id", "ttype", string="Type", type="char",
            readonly=True),
        "relation": fields.related(
            "field_id", "relation", type="char",
            string="Relation", readonly=True),
        "state": fields.related(
            "field_id", "state", string="State", type="char",
            readonly=True),
    }
    ...
    def onchange_field(self, cr, uid, ids, field_id, context=None):
        ...
        value = field_obj.read(cr, uid, field_id,
                               ["name", "ttype", "relation", "state"])
        if value["ttype"] != "one2many":
            value["domain"] = ""
        return {"value": value}
